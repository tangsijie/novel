...
class AccountCountroller {
  ...
  async register(request, response, next) {
    let insertSql = "INSERT INTO `reader`(`rname`,`rpwd`)values(?,?) ; ";
    let params = [request.body.rname, request.body.rpwd, request.body.status];
    let sel_sql = "Select * from `reader` where `rname` = ? ; ";
    if (params[2] == 2) {
      insertSql = "INSERT INTO `writer`(`wname`,`wpwd`)values(?,?) ; ";
