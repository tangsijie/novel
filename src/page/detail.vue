  ...
  <div :class="dark == true ? 'body' : 'body2'">
    ...
    <div :class="mululist1Fixed == true ? 'mululist1' : ''">
      <div :class="mululist2Fixed == true ? 'mululist2' : ''">
        <div @click="showmuluclick">
          <i class="iconfont icon-mulu"></i>
          <p>目录</p>
        </div>
        <div @click="yewan">
          <i class="iconfont icon-yewanmoshi"></i>
          <p>护眼</p>
        </div>
        <div>
        ...
        <div @click="goshuye">
          <i class="iconfont icon-fanhuishuye"></i>
          <p>书页</p>
        </div>
      </div>
    </div>
    ...
    <div class="contianer-top" v-if="fengmian">
      ...
      <h3>
        与<span>{{ book.fangwenliang }}</span
        >位书友共同开启《诸天读书人》的武侠之旅
      </h3>
      <h5>本站由汤圆小说自行创作研究学习</h5>
      <h6>©版权所有 侵权必究</h6>
    </div>
    <div class="contianer-last">
      <div class="contianer-last1">
        <span
          >{{ zhangjie[this.num - 1].zhangjieshu }}
          {{ zhangjie[this.num - 1].title }}</span
        >
        <pre>
    ...
    <div class="comment">
      ...
      <div class="comment-box">
        <div v-for="(item, index) in comshowmeag" :key="index" class="comall">
          ...
          <div class="img-box"><img :src="imgurl" /></div>
          <p class="com-name">{{ item.username }}</p>
          <p class="com-time">
            发表于{{ item.time | dateFmt("YYYY-MM-DD HH:mm:ss") }}
          </p>
          <div class="com-value">{{ item.value }}</div>
          <div
            class="com-button"
            v-if="LoginUser != '' && LoginUser.rname != item.username"
          >
            回复
          </div>
          <div
...
export default {
      ...
      book: 
        {
          ...
          zifenlei: "",
          fufenlei: "历史",
          bookimg: "https://images-cn.ssl-images-amazon.cn/images/I/41v4q9Itz8L.jpg",
          jieshao: "教会你投资",
        }
      ,
      zhangjie: [],
  ...
  methods: {
    ...
    //获取传递数据
    getdata() {
      if(this.$route.params.detailmesg&&this.$route.params.zhangjiemesg){
        this.book = this.$route.params.detailmesg;
        this.zhangjie = this.$route.params.zhangjiemesg;
      }else{
        this.book=this.$store.state.bookdetail.detailmesg
        this.zhangjie =this.$store.state.bookdetail.zhangjiemesg
      }
      console.log("this.zhangjie::",this.zhangjie);
...
.contianer-top > h1 {
  font: 30px/45px PingFangSC-Regular, HelveticaNeue-Light,
    "Helvetica Neue Light", "Microsoft YaHei", sans-serif;
  overflow: hidden;
  height: 45px;
  margin-bottom: 7px;
...
.xinxi > div > p {
  font: 18px/26px PingFangSC-Regular, HelveticaNeue-Light,
    "Helvetica Neue Light", "Microsoft YaHei", sans-serif;
  margin-bottom: 5px;
  white-space: nowrap;
}
...
.contianer-top > h3 {
  font: 16px/26px PingFangSC-Regular, HelveticaNeue-Light,
    "Helvetica Neue Light", "Microsoft YaHei", sans-serif;
  overflow: hidden;
  height: 26px;
  margin-bottom: 8px;
...
.contianer-top > h5 {
  font: 14px/26px PingFangSC-Regular, HelveticaNeue-Light,
    "Helvetica Neue Light", "Microsoft YaHei", sans-serif;
  color: #666;
  margin-top: 20px;
}
...
.bottom > div {
  font: 18px/50px PingFangSC-Regular, HelveticaNeue-Light,
    "Helvetica Neue Light", "Microsoft YaHei", sans-serif;
  width: 33.2%;
  text-align: center;
  border: 1px solid #d8d8d8;
