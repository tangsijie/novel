<template>
	<div>
		...
		<div class="main">
			<el-tabs>
				<el-tab-pane v-for="(item, index) in fenlei" :key="index" :label="item.zifenlei">
					<div class="main2">
						<div class="main2left">
							<div id="pai">
								<h3>本周推荐</h3>
								<ul>
									<li v-for="(item, index) in fenlei" :key="index">
											<div style="color: #262626; ">{{ item.bookname }}</div>
											<div>{{ item.writer }}</div>
									</li>
								</ul>
							</div>
						...
						<div class="main2mid">
							<div class="booklist">
								<div class="books" v-for="(item, index) in fenlei" :key="index"
									@click="godetail(item)">
							...
							<div class="newbook">
								...
								<p v-for="(item, index) in fenlei" :key="index">
						...
						<div class="main2right">
							<div>
								...
								<div style="position: absolute;top: 50%;">
									...
									<marquee style="width: 80%;"><span
											style="font-size: 12px;color: #262626;">【资讯】斗破苍穹新书发布，赶紧来看不容错过，巅峰对决</span>
									</marquee>
								</div>
							</div>
		...
		godetail(b) {
			axios({
				...
				res => {
					...
					this.$store.commit('setfufenlei',b)
					this.$router.push({
						name: 'detail',
						params: {
							detailmesg: b,
