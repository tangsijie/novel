	...
	<div>
		...
		<div class="cont">
			...
			<div class="cont1">
				...
				<div v-else>{{}}</div>
				<div id="code" v-show="isshow"
					style="display:block; position: absolute;right: 208px;top: -30px;background-color:white;width:180px;height: 200px;">
					<div>
						<div>
							<p
								style="border-top:1px solid black;position: absolute;bottom: 130px;left: 50px;padding-top: 10px;">
								汤圆公众号</p>
							<div
								style="position: absolute;width:113px;height:113px;bottom: 0px;left: 35px; background-image: url(../../static/img/code-wx.png);background-size: 100%;">
							</div>
						</div>
					</div>
				</div>
									...
									<div class="lab3">
										...
										<div>
											<div v-for="(item, index) in zhangjie.slice(0, 30)" :key="index">
												<p>{{ item.zhangjieshu }} {{ item.title }}</p>
												<i>-</i><em>{{ item.time.substring(0, 10) }}</em>
											</div>
										</div>
									</div>
								</div>
...
export default {
	...
	methods: {
		...
		tuijianshow() {
			axios({
				...
				res => {
					...
					this.tjs = this.tuijianshowmesg.length
				},
				err => {
					console.log(err.msg);
				}
		...
		isaddtuijianshow() {
			axios({
				...
				res => {
					...
					// console.log('222',this.isaddtuijianmesg);
		...
		godetail() {
			...
			// console.log('1122',this.getBook.bookname);
			// console.log('1122',this.getBook.fangwenliang);
			axios({
				method:'post',
				url:'http://127.0.0.1:3000/getSql/fangwenliang',
				data:{
					bookname:this.getBook.bookname,
					fangwenliang:Number(this.getBook.fangwenliang )+ 1
				}
			}).then(res=>{
				console.log('fangwenliang',res);
			})
		},
